...
@TeleOp(name = "YakirWolfWolf", group = "Iterative Opmode")
@Disabled public class DriveYakirHaKing extends OpMode {
    double posRelic;
    ...
    public void loop() {
        ...
        telemetry.addData("pos", relicDC.getCurrentPosition());
        //        telemetry.update();
        up = touchUp.getState();
        ...
        if (a == 1) {
            ...
            if (gamepad2.x || gamepad2.y) {
                ...
                if (flagSideWait) {
                    ...
                    if (gamepad2.x && runtime.seconds() > 0.4) {
                        flagSideWait = false;
                        flagSideOpen = false;
                    } else if (gamepad2.y) {
                        flagSideWait = false;
            ...
            if (gamepad2.left_bumper) {
                relicServo[1].setPosition(relicServo[1].getPosition() + 0.04);
            }
            ...
            if (gamepad2.right_trigger != 0) {
                if (relicEncoder < 2200) {
                    relicDC.setTargetPosition(relicEncoder + 200);
                    relicDC.setMode(DcMotor.RunMode.RUN_TO_POSITION);
                    relicDC.setPower(0.6);
                }
                relicServo[0].setPosition(0.8);
                relicServo[1].setPosition(0.35);
            }
...
//        drivetrainDC[1][1].setPower(gain * (gamepad1.right_stick_y));
        ...
        drivetrainDC[0][1].setPower((gamepad1.left_stick_x + gamepad1.left_stick_y + 0.8*gamepad1.right_stick_x));
        drivetrainDC[1][1].setPower((-gamepad1.left_stick_x + gamepad1.left_stick_y + 0.8*gamepad1.right_stick_x));
        drivetrainDC[0][0].setPower((-gamepad1.left_stick_x + gamepad1.left_stick_y - 0.8*gamepad1.right_stick_x));
        drivetrainDC[1][0].setPower((gamepad1.left_stick_x + gamepad1.left_stick_y - 0.8*gamepad1.right_stick_x));
    }
